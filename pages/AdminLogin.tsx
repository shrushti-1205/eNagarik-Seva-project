...
const AdminLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    setIsLoading(true);
    const result = await login(emailOrPhone, password, Role.ADMIN);
    setIsLoading(false);
    if (result.success) {
      navigate(ROUTES.ADMIN_DASHBOARD);
    } else {
      setError(t('errors.invalidAdminCredentials'));
    }
  };
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <button
              type="submit"
              disabled={isLoading}
              className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-700 disabled:bg-gray-500"
            >
